...
Semantic = Union{<:Real,AbstractArray{<:Real,1}}
SemanticSet = Union{<:Real,Array{Semantic,1}}

@inline valueSemantic(semantic::Real, index::Int64) = semantic;
@inline valueSemantic(semantic::AbstractArray{<:Real,1}, index::Int64) = @inbounds semantic[index];
...
@inline function any0(x::AbstractArray{<:Real,1})
...
@inline any0(x::Real,y::AbstractArray{<:Real,1}) = x==0 ? any0(y) : false
@inline any0(x::AbstractArray{<:Real,1},y::Real) = any0(y,x)
@inline function any0(x::AbstractArray{<:Real,1},y::AbstractArray{<:Real,1})
    @inbounds for i=1:length(x)
...
@inline function anyInf(x::AbstractArray{<:Real,1})
    @inbounds for i=1:length(x)
...
@inline function anyNaN(x::AbstractArray{<:Real,1})
    @inbounds for i=1:length(x)
...
@inline function all0(x::AbstractArray{<:Real,1})
    @inbounds for i=1:length(x)
        x[i]!=0 && return false
    end
    return true
...
@inline function anyequal(x::Int64, y::AbstractArray{Int64,1})
    @inbounds for i=1:length(y)
        y[i]==x && return true
    end
    return false
...
@inline cleanVector(v::AbstractArray{<:Real,1}) = (allequal(v)) ? v[1] : v
...
function calculateMSEFromEquation(semantic::Semantic, a::Semantic, b::Semantic, c::Semantic, d::Semantic, S::SemanticSet; checkForErrors=false)
    ...
    function calculateMSEFromEquationEfficient()
        if all0(c)
    ...
    function calculateMSEFromEquationMoreEfficient()
        if all0(c)
            (any0(d)) && return NaN;
            if allequal(d)
                # d is a constant
    ...
    # mse = calculateMSEFromEquationMoreEfficient();
    #
    # if checkForErrors
    #     @assert(!isinf(mse));
    #     newMSE2 = calculateMSEFromEquationEfficient();
    #     @assert(isnan(mse) == (isnan(newMSE2) || isinf(newMSE2)))
    #     if (!isnan(mse))
    #         @assert(equal(mse,newMSE2));
    #     end;
    #     newMSE3 = calculateMSEFromEquationVectorial();
    #     @assert(isnan(newMSE2) == isnan(newMSE3));
    #     @assert(isinf(newMSE2) == isinf(newMSE3));
    #     if isinf(newMSE2)
    #         @assert(sign(newMSE2)==sign(newMSE3))
    #     elseif (!isnan(mse))
    #         @assert(equal(mse,newMSE3));
    #     end;
    # end;


    mse = calculateMSEFromEquationEfficient();

    if checkForErrors
        mseV = calculateMSEFromEquationVectorial();
        @assert(isnan(mse) == isnan(mseV));
        @assert(isinf(mse) == isinf(mseV));
        if isinf(mse)
            @assert(sign(mse)==sign(mseV))
        elseif (!isnan(mse))
            @assert(equal(mse,mseV));
        end;
        mseE = calculateMSEFromEquationMoreEfficient();
        @assert(!isinf(mseE));
        @assert(isnan(mseE) == (isnan(mse) || isinf(mse)))
        if (!isnan(mseE))
            @assert(equal(mse,mseE));
        end;
    end;
